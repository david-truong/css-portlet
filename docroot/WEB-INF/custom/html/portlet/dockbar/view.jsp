					...
					<div class="aui-menu-content">
						<ul>
							...
							<c:if test="<%= themeDisplay.isShowPageSettingsIcon() %>">
								<li class="css">
									<%
									PortletURL cssSettingsURL = new PortletURLImpl(
										request, "1_WAR_cssportlet", plid,
										PortletRequest.RENDER_PHASE);

									cssSettingsURL.setWindowState(WindowState.MAXIMIZED);
									cssSettingsURL.setPortletMode(PortletMode.VIEW);
									cssSettingsURL.setParameter("returnToFullPageURL", PortalUtil.getLayoutURL(layout, themeDisplay));
									
									%>
									<a href="<%= cssSettingsURL.toString() %>" style="background-image:url('<%= themeDisplay.getPathThemeImages() %>/portlet/portlet_css.png');">
